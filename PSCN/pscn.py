...
import utils
from PSCN.data_loader import load_local_data
...
if __name__ == '__main__':
    ...
    if hintbot:
        ...
        bzr_dataset = load_local_data('./data', 'hintbot',attributes=False,use_node_deg=True)
        X, y = zip(*bzr_dataset)
        pscn=PSCN(w=10,use_node_deg=True,one_hot=False) #see receptive_field_maker_example for more details
    ...
    if mutag:
        ...
        # %%
        pscn = PSCN(w=18, k=10, one_hot=7, epochs=10, batch_size=32,
                    verbose=2,use_node_deg=True)  # see receptive_field_maker_example for more details
        # %%
