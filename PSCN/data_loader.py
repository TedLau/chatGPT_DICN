...
def load_local_data(data_path, name, one_hot=False, attributes=True, use_node_deg=False):
    ...
    if name == 'hintbot':
        path = data_path + '/HINTBOT/'
        dataset = build_HINTBOT_dataset(path,use_node_deg=use_node_deg)
    if name == 'imdb-b':
...
# %%
def generate_binary_uniform_tree(maxdepth, coupling='cross', a=0, b=5, c=5,
                                 d=10):  # il faut que nlowLeaves soit une puissance de 2
    graph = Graph()
...
def build_IMDB_dataset(path, s='MULTI', use_node_deg=False):
    ...
    return data
def build_HINTBOT_dataset(path,  use_node_deg=False):
    graphs = graph_label_list(path,'IMDB-BINARY_graph_labels.txt')
    adjency = compute_adjency(path, 'IMDB-BINARY_A.txt')
    data_dict = graph_indicator(path, 'IMDB-BINARY_graph_indicator.txt')
    # node_dic=node_labels_dic(path,'MUTAG_node_labels.txt') # ya aussi des nodes attributes ! The fuck ?
    data = []
    for i in graphs:
        g = Graph()
